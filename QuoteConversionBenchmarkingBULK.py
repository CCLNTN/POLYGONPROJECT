...
# 1. Define the base directories for QUOTES data.
BASE_INPUT_DIR = r"Z:\2025\QUOTES\GZFILES"
BASE_OUTPUT_DIR = r"C:\2025\QUOTES\PARQUET"

# 2. Define the explicit schema for the source QUOTES CSV files.
COLUMN_DEFINITIONS = {
    'ticker': 'VARCHAR',
    'ask_exchange': 'BIGINT',
    'ask_price': 'DOUBLE',
...
def convert_file(source_path: Path, output_dir: Path):
    """
    Uses DuckDB to convert a single quotes CSV to a typed, filtered Parquet file.
    """
    # Create a descriptive name for the output file
    destination_file = output_dir / f"{source_path.stem.replace('.csv', '')}_quotes.parquet"
    
    # Build the SELECT clause to cast timestamps after a safe read
    select_clause_parts = []
    ...
    # Format the column definitions into the struct format DuckDB requires
...
if __name__ == "__main__":
    print(f"Starting bulk conversion for QUOTES...")
    ...
    # Get all subdirectories (e.g., '01', '02', ...) in the base directory.
    try:
    ...
    # Sort the folders by name in reverse order ('12', '11', ... '01').
    ...
    # Loop through each folder from most recent to oldest.
    for month_folder in sorted_folders:
        ...
        for source_file in source_files_in_month:
            ...
            output_file = output_sub_dir / f"{source_file.stem.replace('.csv', '')}_quotes.parquet"
